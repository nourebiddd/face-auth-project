...
def encode_faces(data_dir):
    ...
    print(f"[INFO] Encoding {len(people)} synthetic identities...")
...
def main(args):
    ...
    cache = os.path.join(args.out_dir, "encodings_synthetic.pkl")
    ...
    print("[INFO] Training SVM on synthetic data...")
    t0 = time.time()
    ...
    print(f"\n{'='*50}\n  Accuracy (Synthetic Data): {acc*100:.2f}%\n{'='*50}\n")
    print(report)

    with open(os.path.join(args.out_dir,"model_synthetic.pkl"),"wb") as f:
        pickle.dump({"classifier":clf,"label_encoder":le}, f)
    with open(os.path.join(args.out_dir,"metrics_synthetic.txt"),"w") as f:
        f.write(f"Accuracy: {acc*100:.2f}%\n\n{report}")
    with open(os.path.join(args.out_dir,"summary_synthetic.json"),"w") as f:
        json.dump({"data_type":"synthetic","n_samples":int(X.shape[0]),
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(args.out_dir,"confusion_matrix_synthetic.png"), dpi=150)
    print(f"\n[INFO] All results saved to {args.out_dir}/")

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--data_dir", default="data/synthetic_raw")
    parser.add_argument("--out_dir", default="results/synthetic")
